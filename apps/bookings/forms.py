...
class BookingForm(forms.ModelForm):
    preferred_language = forms.ModelChoiceField(
        ...
        required=True,
        empty_label=_("Selecciona un idioma"),
    ...
    def __init__(self, *args, experience=None, **kwargs):
        ...
        if not allowed_qs.exists():
            self.fields["preferred_language"].help_text = _("Este guía aún no ha configurado idiomas.")

        # -----------------------------
        # Pickup notes según transporte
        # -----------------------------
        self.fields["pickup_notes"].label = _("Tu ubicación o referencia")
        transport = getattr(experience, "transport_requirement", None)

        if transport == "own_vehicle":
            self.fields["pickup_notes"].help_text = _(
            ...
            self.fields["pickup_notes"].widget.attrs["placeholder"] = _("Ej: Parking Mirador del Río / Gasolinera X")
        elif transport == "bicycle":
            self.fields["pickup_notes"].help_text = _(
                "Indica dónde te alojas o una referencia de tu ubicación para que el guía organice la logística. "
                "El guía te confirmará el punto de encuentro definitivo al aceptar la reserva."
            )
            self.fields["pickup_notes"].widget.attrs["placeholder"] = _("Ej: Plaza central / Tienda de bicis X")
        else:
            self.fields["pickup_notes"].help_text = _(
                "Indica dónde te alojas o una referencia de tu ubicación para que el guía organice la logística. "
                "El guía te confirmará el punto de encuentro definitivo al aceptar la reserva."
            )
            self.fields["pickup_notes"].widget.attrs["placeholder"] = _("Ej: Entrada principal / Punto exacto en Maps")
    ...
    class Meta:
        ...
        widgets = {
            ...
            "notes": forms.Textarea(
                attrs={
                    "rows": 3,
                    "placeholder": _("Opcional: alergias, ritmo, restricciones, necesidades, etc."),
                }
            ),
    ...
    def clean_preferred_language(self):
        ...
        if not lang:
            raise forms.ValidationError(_("Selecciona el idioma preferido para la experiencia."))
        ...
        if not guide_profile:
            raise forms.ValidationError(_("El guía aún no ha configurado idiomas para esta experiencia."))

        allowed_ids = set(guide_profile.languages.values_list("id", flat=True))
        if lang.id not in allowed_ids:
            raise forms.ValidationError(_("Ese idioma no está disponible para este guía."))
    ...
    def clean(self):
        ...
        if adults <= 0:
            self.add_error("adults", _("Debe haber al menos 1 adulto."))

        people = adults + children + infants
        if people <= 0:
            raise forms.ValidationError(_("Debes indicar al menos 1 persona."))
        ...
        if date_value:
            ...
            if date_value < today:
                self.add_error("date", _("No puedes reservar en una fecha pasada."))

            if date_value <= today + timedelta(days=1):
                self.add_error("date", _("No se permiten reservas con menos de 24 hrs de antelación."))
        ...
        if not pickup_notes:
            self.add_error("pickup_notes", _("Indica tu ubicación o una referencia (el guía te confirmará el punto de encuentro exacto)."))

        if self.errors:
            raise forms.ValidationError(_("Revisa el formulario: hay campos con errores."))
...
class BookingDecisionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["meeting_point"].label = _("Punto de encuentro")
        self.fields["meeting_point"].help_text = _("Indica el punto exacto donde os encontraréis.")
    ...
    def clean(self):
        ...
        if self.require_guide_response and not guide_response:
            self.add_error("guide_response", _("Indica un motivo para rechazar la reserva."))

        if self.require_pickup_time and not pickup_time:
            self.add_error("pickup_time", _("Indica la hora de recogida/encuentro para aceptar la reserva."))

        if self.require_pickup_time:
            if not meeting_point and not guide_response:
                raise forms.ValidationError(
                    _("Para aceptar, indica al menos el punto de encuentro o un mensaje para el viajero.")
                )
...
class BookingChangeRequestForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {
            ...
            "children": _("Niños"),
            "infants": _("Bebés"),
            "pickup_notes": _("Ubicación o referencia"),
            "notes": _("Notas para el guía"),
        }
        widgets = {
